...
class TfidfTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        if self.word_vec_ is None or self.char_vec_ is None:
            raise ValueError("TF-IDF not fitted or loaded")
        if isinstance(X, pd.Series):
            X = pd.DataFrame({"comment_text": X})
        texts = X[TEXT_COL].fillna("").astype(str).tolist()
        return hstack([
...
class BertTransformer(BaseEstimator, TransformerMixin):
    ...
    def _get_cache_path(self, split):
        return os.path.join(self.cache_dir, f"bert_{split}.pkl")
    ...
    def transform(self, X, split="train", use_emb=True):
        if use_emb:
            cached = self._load_emb(split)
...
class FeatureBuilder:
    def __init__(self, cache_dir="./data/processed"):
        ...
        self.tfidf_path = os.path.join(cache_dir, "tfidf.pkl")
        
        self.tfidf = TfidfTransformer()
        self.dense = DenseFeatureTransformer()
        self.bert = BertTransformer(cache_dir=cache_dir)

        self.is_fitted = False

    def fit(self, X_train):
        print("Fitting TF-IDF transformers...")

        self.tfidf.fit(X_train)

        if not hasattr(self.tfidf, "word_vec_") or not hasattr(self.tfidf, "char_vec_"):
            raise ValueError("TFIDF not fitted properly")
    ...
    def load(self): 
        ## for inference mode, load cached transformers from disk 
        # and set them as attributes
        if not os.path.exists(self.tfidf_path):
            raise ValueError("No cached feature transformers found. Train first.")
    ...
    def transform(self, X, split="train", use_cached_tfidf=True):
        ...
        if "comment_text" not in X.columns:
            raise KeyError("Expected column: comment_text")
        
        X_dense = self.dense.transform(X)
        #X_tfidf = self.tfidf.transform(X)

        #print("Saving TF-IDF to", os.path.join(self.cache_dir, f"tfidf_{split}.npz"))
        #save_npz(os.path.join(self.cache_dir, f"tfidf_{split}.npz"), X_tfidf)

        os.makedirs(self.cache_dir, exist_ok=True)
        tfidf_path = os.path.abspath(os.path.join(self.cache_dir, f"tfidf_{split}.npz"))

        if use_cached_tfidf and os.path.exists(tfidf_path):
            print("Loading saved TF-IDF from:", tfidf_path)
            X_tfidf = load_npz(tfidf_path)
        else:
            print("Computing TF-IDF and saving to:", tfidf_path)
            X_tfidf = self.tfidf.transform(X)
            save_npz(tfidf_path, X_tfidf)

        X_bert = self.bert.transform(X, split=split)
...
class FeaturePreprocessor(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.scaler_dense = MinMaxScaler()
        self.scaler_bert = MinMaxScaler()

    def fit(self, X, y=None):
        self.scaler_dense.fit(X["dense"])
        self.scaler_bert.fit(X["bert"])
        return self

    def transform(self, X):
        dense = self.scaler_dense.transform(X["dense"])
        bert = self.scaler_bert.transform(X["bert"])
        tfidf = X["tfidf"]

        return hstack([dense, tfidf, bert])